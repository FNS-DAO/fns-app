<template>
  <div class="h-13 flex flex-center items-stretch border border-primary rounded-full">
    <input
      v-model.trim="input"
      type="text"
      placeholder="Input address or domain"
      class="flex-1 px-5 text-lg text-primary border-transparent outline-none font-mono transition bg-transparent"
      @keydown.enter="search"
    />
    <div class="flex-center cursor-pointer rounded-r-md px-5 text-primary">
...
function search() {
  ...
  if (route.params?.id === searchContent.value) {
    const event = inputType.value === SEARCH_TYPE_ADDRESS ? 'update_address' : 'update_name'
    eventHub.emit(event)
    return
  }
